...
void destroy_context(ze_context_handle_t context) { EXPECT_TRUE(ZE_RESULT_SUCCESS == zeContextDestroy(context)); }
...
void* allocate_host_memory(const size_t size, const size_t alignment, const ze_context_handle_t context) {
  ...
  void* memory = nullptr;
  EXPECT_EQ(ZE_RESULT_SUCCESS, zeMemAllocHost(context, &host_desc, size, alignment, &memory));
...
void* allocate_device_memory(const size_t size, const size_t alignment, const ze_device_mem_alloc_flags_t flags,
  ...
  EXPECT_EQ(ZE_RESULT_SUCCESS, zeMemAllocDevice(context, &device_desc, size, alignment, device_handle, &memory));
  EXPECT_NE(nullptr, memory);
...
void* allocate_shared_memory(const size_t size, const size_t alignment, const ze_device_mem_alloc_flags_t dev_flags,
  ...
  host_desc.pNext = nullptr;
  EXPECT_EQ(ZE_RESULT_SUCCESS, zeMemAllocShared(context, &device_desc, &host_desc, size, alignment, device, &memory));
  EXPECT_NE(nullptr, memory);

  return memory;
}

void free_memory(ze_context_handle_t context, void* ptr) { EXPECT_EQ(ZE_RESULT_SUCCESS, zeMemFree(context, ptr)); }
...
void append_memory_copy(ze_command_list_handle_t cl, void* dstptr, const void* srcptr, size_t size,
  ...
  EXPECT_EQ(ZE_RESULT_SUCCESS,
            zeCommandListAppendMemoryCopy(cl, dstptr, srcptr, size, hSignalEvent, num_wait_events, wait_events));
}
...
ze_module_handle_t create_module(ze_context_handle_t context, ze_device_handle_t device, uint8_t* data, size_t bytes,
  ...
  EXPECT_TRUE((format == ZE_MODULE_FORMAT_IL_SPIRV) || (format == ZE_MODULE_FORMAT_NATIVE));
  ...
  EXPECT_EQ(ZE_RESULT_SUCCESS, zeModuleCreate(context, device, &module_description, &module, p_build_log));

  return module;
}

void destroy_module(ze_module_handle_t module) { EXPECT_EQ(ZE_RESULT_SUCCESS, zeModuleDestroy(module)); }
...
ze_kernel_handle_t create_function(ze_module_handle_t module, ze_kernel_flags_t flag, std::string func_name) {
  ...
  EXPECT_EQ(ZE_RESULT_SUCCESS, zeKernelCreate(module, &kernel_description, &kernel));
  return kernel;
}

void set_argument_value(ze_kernel_handle_t hFunction, uint32_t argIndex, size_t argSize, const void* pArgValue) {
  EXPECT_EQ(ZE_RESULT_SUCCESS, zeKernelSetArgumentValue(hFunction, argIndex, argSize, pArgValue));
}
...
void append_launch_function(ze_command_list_handle_t hCommandList, ze_kernel_handle_t hFunction,
  ...
  EXPECT_EQ(ZE_RESULT_SUCCESS, zeCommandListAppendLaunchKernel(hCommandList, hFunction, pLaunchFuncArgs, hSignalEvent,
                                                               numWaitEvents, phWaitEvents));
}

void destroy_function(ze_kernel_handle_t kernel) { EXPECT_EQ(ZE_RESULT_SUCCESS, zeKernelDestroy(kernel)); }
...
ze_command_list_handle_t create_command_list(ze_context_handle_t context, ze_device_handle_t device,
  ...
  EXPECT_EQ(ZE_RESULT_SUCCESS, zeCommandListCreate(context, device, &descriptor, &command_list));
  EXPECT_NE(nullptr, command_list);

  return command_list;
}

void close_command_list(ze_command_list_handle_t cl) { EXPECT_EQ(ZE_RESULT_SUCCESS, zeCommandListClose(cl)); }
...
void execute_command_lists(ze_command_queue_handle_t cq, uint32_t numCommandLists,
  ...
  EXPECT_EQ(ZE_RESULT_SUCCESS, zeCommandQueueExecuteCommandLists(cq, numCommandLists, phCommandLists, hFence));
}

void reset_command_list(ze_command_list_handle_t cl) { EXPECT_EQ(ZE_RESULT_SUCCESS, zeCommandListReset(cl)); }

void destroy_command_list(ze_command_list_handle_t cl) { EXPECT_EQ(ZE_RESULT_SUCCESS, zeCommandListDestroy(cl)); }
...
ze_command_queue_handle_t create_command_queue(ze_context_handle_t context, ze_device_handle_t device,
  ...
  EXPECT_EQ(ZE_RESULT_SUCCESS, zeCommandQueueCreate(context, device, &descriptor, &command_queue));
...
void destroy_command_queue(ze_command_queue_handle_t cq) { EXPECT_EQ(ZE_RESULT_SUCCESS, zeCommandQueueDestroy(cq)); }
...
zeEventPool::~zeEventPool() {
  if (event_pool_) {
    ...
    if (ZE_RESULT_SUCCESS != result) {
      IVLOG(1, "Failed to destroy event pool " + std::to_string(static_cast<int>(result)));
    }
  }
}
...
void zeEventPool::InitEventPool(ze_context_handle_t context, uint32_t count, ze_event_pool_flags_t flags) {
  ...
  if (event_pool_ == nullptr) {
    ...
    EXPECT_EQ(ZE_RESULT_SUCCESS, zeEventPoolCreate(context, &descriptor, 0, nullptr, &event_pool_));
...
void append_barrier(ze_command_list_handle_t cl, ze_event_handle_t hSignalEvent, uint32_t numWaitEvents,
  ...
  EXPECT_EQ(ZE_RESULT_SUCCESS, zeCommandListAppendBarrier(cl, hSignalEvent, numWaitEvents, phWaitEvents));
}

// Group
void set_group_size(ze_kernel_handle_t hFunction, uint32_t groupSizeX, uint32_t groupSizeY, uint32_t groupSizeZ) {
  EXPECT_EQ(ZE_RESULT_SUCCESS, zeKernelSetGroupSize(hFunction, groupSizeX, groupSizeY, groupSizeZ));
}
...
void suggest_group_size(ze_kernel_handle_t hFunction, uint32_t globalSizeX, uint32_t globalSizeY, uint32_t globalSizeZ,
  ...
  EXPECT_EQ(ZE_RESULT_SUCCESS, zeKernelSuggestGroupSize(hFunction, globalSizeX, globalSizeY, globalSizeZ, &groupSizeX,
