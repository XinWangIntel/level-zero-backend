...
class LevelZeroQueueGuard {
 public:
  explicit LevelZeroQueueGuard(LevelZeroQueue* queue, bool init = false) : used(init), queue(queue) {}
...
/// to device.
class LevelZeroDevice final : public pmlc::rt::Device, public std::enable_shared_from_this<LevelZeroDevice> {
 public:
  explicit LevelZeroDevice(ze_driver_handle_t driver, ze_device_handle_t device);
  ~LevelZeroDevice();

  std::unique_ptr<Executable> compile(const std::shared_ptr<pmlc::compiler::Program>& program) final;
