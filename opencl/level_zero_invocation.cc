...
void LevelZeroMemory::enqueueRead(ze_command_list_handle_t list, void* dst,
  ...
  lzu::append_memory_copy(list, dst, buffer, bytes, resultE, dependencies.size(), dependencies.data());
}

void LevelZeroMemory::enqueueWrite(ze_command_list_handle_t list, void* src,
                                   std::vector<ze_event_handle_t>& dependencies, ze_event_handle_t& resultE) {
  lzu::append_memory_copy(list, buffer, src, bytes, resultE, dependencies.size(), dependencies.data());
}

LevelZeroKernel::LevelZeroKernel(ze_module_handle_t module, std::string name) : module(module), name(name) {
...
void LevelZeroKernel::addDependency(LevelZeroEvent* event) { dependencies.push_back(event->getEvent()); }
...
void LevelZeroKernel::enqueue(ze_command_list_handle_t list, ze_group_count_t gws, ze_group_count_t lws,
                              ze_event_handle_t& resultE) {
  lzu::append_launch_function(list, kernel, &gws, resultE, dependencies.size(), dependencies.data());
  dependencies.clear();
  UNUSED_VARIABLE(lws);
}

LevelZeroEvent::LevelZeroEvent(ze_event_handle_t event, LevelZeroActionKind kind, std::string name)
...
LevelZeroMemory* LevelZeroInvocation::allocateMemory(size_t bytes) {
  ...
  void* buffer = lzu::allocate_shared_memory(bytes, /*alignment*/ 1, /*dev_flags*/ 0, /*host_flags*/ 0,
...
LevelZeroEvent* LevelZeroInvocation::enqueueRead(LevelZeroMemory* src, void* dst,
...
LevelZeroEvent* LevelZeroInvocation::enqueueWrite(LevelZeroMemory* dst, void* src,
                                                  const std::vector<LevelZeroEvent*>& deps) {
  std::vector<ze_event_handle_t> dependencies;
  std::transform(deps.begin(), deps.end(), std::back_inserter(dependencies),
                 [](const LevelZeroEvent* event) { return event->getEvent(); });
...
LevelZeroKernel* LevelZeroInvocation::createKernelFromIL(char* data, size_t bytes, const char* name) {
  uint8_t* buf = reinterpret_cast<uint8_t*>(data);
  ze_module_handle_t module =
      lzu::create_module(device->getLevelZeroContext(), device->getLevelZeroDevice(), buf, bytes,
                         ZE_MODULE_FORMAT_IL_SPIRV, /*build_flags*/ "", /*p_build_log*/ nullptr);

  return new LevelZeroKernel(module, name);
}

LevelZeroEvent* LevelZeroInvocation::enqueueKernel(LevelZeroKernel* kernel, ze_group_count_t gws,
                                                   ze_group_count_t lws) {
  // In LevelZero, gws is group count, lws is group size.
  lzu::set_group_size(kernel->getKernel(), lws.groupCountX, lws.groupCountY, lws.groupCountZ);
  ze_event_handle_t event;
  eventPool.create_event(event);
  kernel->enqueue(queueUser.getLevelZeroList(), gws, lws, event);
  LevelZeroEvent* result = wrapEvent(event, LevelZeroActionKind::Kernel, kernel->getName());
  return result;
}

LevelZeroEvent* LevelZeroInvocation::enqueueBarrier(const std::vector<LevelZeroEvent*>& deps) {
  ze_event_handle_t result;
  eventPool.create_event(result);
  std::vector<ze_event_handle_t> dependencies;
  std::transform(deps.begin(), deps.end(), std::back_inserter(dependencies),
                 [](const LevelZeroEvent* event) { return event->getEvent(); });
  lzu::append_barrier(queueUser.getLevelZeroList(), result, dependencies.size(), dependencies.data());
...
void LevelZeroInvocation::flush() {
  ...
  lzu::execute_command_lists(command_queue, /*numCommandLists*/ 1, &command_list, /*hfence*/ nullptr);
  lzu::synchronize(command_queue, /*timeout*/ UINT64_MAX);
  lzu::reset_command_list(command_list);
}

void LevelZeroInvocation::finish() { lzu::synchronize(queueUser.getLevelZeroQueue(), /*timeout*/ UINT64_MAX); }

LevelZeroEvent* LevelZeroInvocation::wrapEvent(ze_event_handle_t event, LevelZeroActionKind kind, std::string name) {
  events.emplace_back(std::make_unique<LevelZeroEvent>(event, kind, std::move(name)));
