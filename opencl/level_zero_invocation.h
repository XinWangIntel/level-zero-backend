...
class LevelZeroMemory {
 public:
  ...
  void enqueueRead(ze_command_list_handle_t list, void* dst, std::vector<ze_event_handle_t>& dependencies,
                   ze_event_handle_t& resultE);
  /// Enqueues write operation from `src` pointer into this buffer
  /// on specified command queue.
  void enqueueWrite(ze_command_list_handle_t list, void* src, std::vector<ze_event_handle_t>& dependencies,
                    ze_event_handle_t& resultE);
...
class LevelZeroKernel {
 public:
  ...
  void enqueue(ze_command_list_handle_t list, ze_group_count_t gws, ze_group_count_t lws, ze_event_handle_t& resultE);
...
class LevelZeroInvocation {
 public:
  ...
  LevelZeroEvent* enqueueRead(LevelZeroMemory* src, void* dst, const std::vector<LevelZeroEvent*>& deps);
  ...
  /// events are finished.
  /// Returns event describing status of this operation.
  LevelZeroEvent* enqueueWrite(LevelZeroMemory* dst, void* src, const std::vector<LevelZeroEvent*>& deps);
  ...
  LevelZeroEvent* enqueueKernel(LevelZeroKernel* kernel, ze_group_count_t gws, ze_group_count_t lws);
  /// Not used now
 ...
 private:
  ...
  LevelZeroEvent* wrapEvent(ze_event_handle_t event, LevelZeroActionKind kind, std::string name);
};
