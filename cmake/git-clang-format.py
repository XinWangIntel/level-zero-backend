...
def main():
    ...
    if old_tree == new_tree:
        ...
        changed_files = apply_changes(old_tree,
                                      new_tree,
                                      force=opts.force,
        ...
        if match:
            ...
            if line_count > 0:
                matches.setdefault(filename,
                                   []).append(Range(start_line, line_count))
    return matches
...
def clang_format_to_blob(filename,
                         line_ranges,
                         revision=None,
                         binary='clang-format',
                         style=None):
  ...
  Returns the object ID (SHA-1) of the created blob."""
    ...
    if revision:
        ...
        git_show = subprocess.Popen(git_show_cmd,
                                    stdin=subprocess.PIPE,
                                    stdout=subprocess.PIPE)
    ...
    hash_object_cmd = [
        'git', 'hash-object', '-w', '--path=' + filename, '--stdin'
    ]
  ...
  `patch_mode`, runs `git checkout --patch` to select hunks interactively."""
    ...
    if not force:
        ...
        if unstaged_files:
            ...
            print('Please commit, stage, or stash them first.',
                  file=sys.stderr)
            sys.exit(2)
    if patch_mode:
...
def run(*args, **kwargs):
    ...
    if p.returncode == 0:
        if stderr:
            if verbose:
                print('`%s` printed to stderr:' % ' '.join(args),
                      file=sys.stderr)
        ...
        return stdout
    if verbose:
        print('`%s` returned %s' % (' '.join(args), p.returncode),
              file=sys.stderr)
    if stderr:
        print(stderr.rstrip(), file=sys.stderr)
    sys.exit(2)
